#C:\Users\Sayantan Pal\Unity\ml-agents\ml-agents\mlagents\trainers\torch_entities\model_serialization.py
...
class ModelSerializer:
    def __init__(self, policy):
        ...
        if self.policy.export_memory_size > 0:
            ...
            # recurrent_out is also transposed in LSTM.forward() during export
            # causing the batch dimension to be at index 0.
            self.dynamic_axes[TensorNames.recurrent_output] = {0: "batch"}
    ...
    def export_policy_model(self, output_filepath: str) -> None:
        ...
        # Move to CPU for export to avoid Windows-specific CUDA ONNX export crashes
        original_device = next(self.policy.actor.parameters()).device
        self.policy.actor.to("cpu")
        cpu_dummy_input = (
            [t.to("cpu") for t in self.dummy_input[0]],
            self.dummy_input[1].to("cpu"),
            self.dummy_input[2].to("cpu"),
        )
        ...
        self.policy.actor.to(original_device)
